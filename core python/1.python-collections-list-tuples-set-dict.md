...
2. [Lists `[]` - Mutable, Ordered](#1️⃣-lists---mutable-ordered)
   - Syntax & Operations
   - Real-World Use Cases
   - Key Rules & Best Practices
3. [Tuples `()` - Immutable, Ordered](#2️⃣-tuples---immutable-ordered)
   - Syntax & Operations
   - Real-World Use Cases
   - Key Rules & Best Practices
4. [Sets `{}` - Mutable, Unordered, Unique](#3️⃣-sets---mutable-unordered-unique)
   - Syntax & Operations
   - Real-World Use Cases
   - Key Rules & Best Practices
5. [Dictionaries `{}` - Key-Value Pairs](#4️⃣-dictionaries---key-value-pairs)
   - Syntax & Operations
   - Real-World Use Cases
   - Key Rules & Best Practices
6. [Type Casting Between Collections](#-type-casting-between-collections)
   - All Conversion Possibilities
   - When to Convert
   - Common Patterns
7. [When to Use Which Collection](#-when-to-use-which-collection)
8. [Quick Syntax Reference](#-quick-syntax-reference)
...
| **Order preserved** | ✅ Yes | ✅ Yes | ❌ No | ✅ Yes (Python 3.7+) |
| **Indexing** | ✅ Yes | ✅ Yes | ❌ No | ✅ Yes (by key) |
| **Slicing** | ✅ Yes | ✅ Yes | ❌ No | ❌ No |
| **Duplicates** | ✅ Yes | ✅ Yes | ❌ No | ❌ No (keys) |
| **Mutable** | ✅ Yes | ❌ No | ✅ Yes | ✅ Yes |
| **Allowed types** | Any | Any | Immutable only | Keys: Immutable<br>Values: Any |
| | | | | |
| **📖 EXTRACT/READ** | | | | |
| By index/key | `l[0]` first<br>`l[-1]` last | `t[0]` first<br>`t[-1]` last | ❌ Not possible | `d["key"]`<br>`d.get("key")` |
| Slicing | `l[1:3]`<br>`l[::2]` | `t[1:3]`<br>`t[::2]` | ❌ Not possible | ❌ Not possible |
...
| | | | | |
| **➕ ADD ELEMENTS** | | | | |
| Add one | `l.append(5)` (end)<br>`l.insert(2, 5)` (index) | ❌ Immutable | `s.add(5)` | `d["key"] = value`<br>`d.update({"key": v})` |
| Add multiple | `l.extend([5, 6])`<br>`l += [5, 6]` | ❌ Immutable | `s.update([5, 6, 7])`<br>`s.update({5, 6})` | `d.update({"k1": 1, "k2": 2})` |
| | | | | |
| **✏️ MODIFY/REPLACE** | | | | |
| Replace one | `l[0] = 10`<br>`l[-1] = 20` | ❌ Immutable | ❌ Not possible<br>(remove & add) | `d["key"] = new_value` |
| Replace multiple | `l[1:3] = [10, 20]` | ❌ Immutable | ❌ Not possible | `d.update({"k1": 10, "k2": 20})` |
| | | | | |
| **🗑️ REMOVE ELEMENTS** | | | | |
| Remove by value | `l.remove(5)` (first)<br>⚠️ ValueError if missing | ❌ Immutable | `s.remove(5)` ⚠️ KeyError<br>`s.discard(5)` ✅ Safe | ❌ Not applicable |
| Remove by index/key | `l.pop(0)` (returns)<br>`del l[0]` | ❌ Immutable | `s.pop()` (arbitrary) | `d.pop("key")` (returns)<br>`del d["key"]` |
| Remove last | `l.pop()` | ❌ Immutable | `s.pop()` (arbitrary) | `d.popitem()` (last pair) |
| Remove all | `l.clear()` → `[]` | ❌ Immutable | `s.clear()` → `set()` | `d.clear()` → `{}` |
| Delete variable | `del l` | `del t` | `del s` | `del d` |
| | | | | |
| **🔧 OTHER OPERATIONS** | | | | |
| Length | `len(l)` | `len(t)` | `len(s)` | `len(d)` (keys) |
| Count occurrences | `l.count(5)` | `t.count(5)` | ❌ (always 0 or 1) | ❌ (keys unique) |
| Find index | `l.index(5)` | `t.index(5)` | ❌ Not possible | ❌ Not possible |
| Sort | `l.sort()` in-place<br>`sorted(l)` new | `sorted(t)` → list | `sorted(s)` → list | `sorted(d)` keys<br>`sorted(d.items())` |
| Reverse | `l.reverse()` in-place<br>`l[::-1]` new | `t[::-1]` new tuple | ❌ Not ordered | ❌ Not typical |
...
❌ Using `{}` for empty set (it's a dict!)
❌ Trying to modify tuples
❌ Putting mutable types in sets
❌ Using mutable types as dict keys
...
✅ Use `tuple` for fixed data and function returns
✅ Use `set` for uniqueness and fast lookups
✅ Use `dict` for structured data with named fields
✅ Use `get()` with dicts to avoid KeyErrors
✅ Use `discard()` with sets to avoid KeyErrors
...
# Remove by index
l.pop(0)               # Removes 10, returns 10 → [30, 20, 40]
l.pop()                # Removes 40, returns 40 → [30, 20]
del l[0]               # Removes 30 → [20]
...
❌ **Avoid Lists When:**
- You need fast membership testing (`in` operator) → Use Set
- You need key-value mapping → Use Dictionary
- Data should be immutable/protected → Use Tuple
...
#### ➕ Add / ✏️ Modify / 🗑️ Remove
```python
# ❌ ALL MODIFICATION OPERATIONS ARE NOT ALLOWED
# Tuples are immutable!

t = (10, 20, 30)
# t.append(40)      # ❌ AttributeError
# t[0] = 100        # ❌ TypeError
# t.remove(20)      # ❌ AttributeError

# ✅ Workarounds (create new tuple):
t = t + (40,)     # (10, 20, 30, 40)
t = (5,) + t      # (5, 10, 20, 30, 40)
```
...
## 3️⃣ SETS `{}` - Mutable, Unordered, Unique
...
#### ✏️ Modify/Replace
```python
# ❌ Cannot replace directly (no indexing)
...
# Remove by value
s.remove(20)           # {10, 30, 40, 50} ⚠️ KeyError if not found
s.discard(30)          # {10, 40, 50} ✅ No error if not found
...
- `{0, False}` → `{0}` (bool False == int 0)
- `{1, True}` → `{1}` (bool True == int 1)
...
# Single value by key
d["name"]              # "Alice" ⚠️ KeyError if not found
d.get("age")           # 30
...
- You only need values without keys → Use List/Set
- You need ordered sequence by index → Use List
- Keys are not unique or immutable → Cannot use dict
...
## 🔄 Type Casting Between Collections

### Complete Conversion Matrix

| From ↓ To → | List | Tuple | Set | Dictionary |
|-------------|------|-------|-----|------------|
| **List** | `l` (itself) | `tuple(l)` | `set(l)` ✅ | `dict(l)` ⚠️ |
| **Tuple** | `list(t)` ✅ | `t` (itself) | `set(t)` ✅ | `dict(t)` ⚠️ |
| **Set** | `list(s)` ✅ | `tuple(s)` ✅ | `s` (itself) | ❌ Not directly |
| **Dictionary** | `list(d)` keys<br>`list(d.values())`<br>`list(d.items())` | `tuple(d)` keys<br>`tuple(d.items())` | `set(d)` keys | `d` (itself) |

⚠️ **Note:** List/Tuple → Dict requires list of tuples with 2 elements each

### 1️⃣ List Conversions
...
# ❌ FAILS if list contains mutable types:
l = [1, 2, [3, 4]]
...
#### List → Dictionary
```python
# ✅ From list of tuples (each with 2 elements)
l = [("a", 1), ("b", 2), ("c", 3)]
...
# ❌ FAILS if elements don't have exactly 2 items:
l = [(1, 2, 3)]
...
#### Set → Dictionary
```python
# ❌ No direct conversion (sets don't have key-value pairs)
...
#### Pattern 3: Count Occurrences (List → Dict)
```python
items = ["a", "b", "a", "c", "b", "a"]
...
### ⚠️ Conversion Rules & Gotchas

**1. Set Conversion Rules:**
```python
# ✅ Only immutable types in set
s = {1, 2, (3, 4)}     # OK
...
```python
# ✅ Each element must have exactly 2 items
l = [("a", 1), ("b", 2)]
d = dict(l)  # OK
...
| Product inventory | **Dictionary** | Product ID → details mapping |
